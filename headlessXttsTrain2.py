...
def preprocess_dataset_headless(audio_file_path, language, whisper_model_name, dataset_out_path, whisper_model_root):
    ...
    print(f"Whisper model root path: {whisper_model_root}")
    ...
    try:
        ...
        asr_model = WhisperModel(
            ...
            download_root=whisper_model_root,
        )
...
def main():
    ...
    parser.add_argument("--whisper_model_root", type=str, default=None, help="Directory where the Whisper models should be saved. If not set, the models are saved in the standard Hugging Face cache directory.")
    ...
    parser.add_argument("--clean_run_dir", action="store_true", help="Remove `run` directory containing the intermediate checkpoints.")
    ...
    parser.add_argument("--skip_generation", action="store_true", help="Do not generate example TTS after training is completed.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # --- Step 2: Training ---
        status, config_path, vocab_path, unoptimized_model_path, speaker_xtts_path, original_speaker_wav, run_dir = train_model_headless(
             language=args.lang,
        ...
        # Delete "run" folder if needed
        if args.clean_run_dir and run_dir.exists():
            print(f"Post-Removing training run directory: {run_dir}")
            shutil.rmtree(run_dir)
